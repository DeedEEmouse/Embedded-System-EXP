...
/* Definitions for buttonTime */
osMessageQueueId_t buttonTimeHandle;
const osMessageQueueAttr_t buttonTime_attributes = {.name = "buttonTime"};
/* Definitions for ledMutex */
osMutexId_t ledMutexHandle;
const osMutexAttr_t ledMutex_attributes = {.name = "ledMutex"};
/* Definitions for button */
osSemaphoreId_t buttonHandle;
const osSemaphoreAttr_t button_attributes = {.name = "button"};
/* Definitions for timer */
osSemaphoreId_t timerHandle;
const osSemaphoreAttr_t timer_attributes = {.name = "timer"};
/* USER CODE BEGIN PV */
...
int main(void) {
  ...
  /* Create the queue(s) */
  /* creation of buttonTime */
  buttonTimeHandle =
      osMessageQueueNew(16, sizeof(uint16_t), &buttonTime_attributes);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
  ...
  if (GPIO_Pin == GPIO_PIN_13) {
    ...
    if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_SET) {
      ...
      osMessageQueuePut(buttonTimeHandle, &deltatime, 0, 0);
    }
  } else {
    __NOP();
  }
...
void StartTask_1(void *argument) {
  ...
  /* Infinite loop */
  for (;;) {
    // osSemaphoreAcquire(buttonHandle, osWaitForever);
    uint32_t deltatime = 0;
    osMessageQueueGet(buttonTimeHandle, &deltatime, 0, osWaitForever);
...
void StartTask_2(void *argument) {
  ...
  /* Infinite loop */
  for (;;) {
    osSemaphoreAcquire(timerHandle, osWaitForever);
    osMutexAcquire(ledMutexHandle, osWaitForever);
    for (int i = 0; i < 20; i++) {
      HAL_GPIO_WritePin(GPIOB, LED2_Pin, GPIO_PIN_SET);
      osDelay(50);
